...
SUSPICIOUS_TLDS = {"zip", "mov", "top", "xyz", "click", "country", "gq", "tk", "work"}
...
def analyze(content: str) -> tuple[int, str, str, list[Finding]]:
    ...
    if matched_terms:
        ...
        _add(findings, "Social-engineering language", f"Detected: {', '.join(matched_terms[:4])}.", points)

    if re.search(r"\b(?:act now|immediately|within \d+ hours?|final warning)\b", lower):
        _add(findings, "Pressure tactic", "The message tries to create urgency or fear.", 14)
    if re.search(r"\b(?:password|pin|cvv|security code|login credentials)\b", lower):
        _add(findings, "Sensitive-data request", "It mentions information legitimate services should not request by message.", 18)
    if value.count("!") >= 3 or re.search(r"\b[A-Z]{5,}\b", value):
        _add(findings, "Manipulative formatting", "Excessive exclamation marks or all-caps wording can signal pressure.", 7)
    ...
    for raw_url in urls[:3]:
        ...
        if parsed.scheme == "http":
            _add(findings, "Unencrypted link", f"{raw_url} uses HTTP rather than HTTPS.", 12)
        if host in SHORTENERS:
            _add(findings, "Shortened link", f"{host} hides the final destination.", 18)
        if re.fullmatch(r"\d{1,3}(?:\.\d{1,3}){3}", host):
            _add(findings, "Raw IP address", "The link uses an IP address instead of a recognizable domain.", 22)
        if "xn--" in host:
            _add(findings, "Look-alike domain", "The link uses punycode, which can impersonate familiar names.", 24)
        if suffix in SUSPICIOUS_TLDS:
            _add(findings, "Higher-risk domain ending", f".{suffix} is frequently abused in low-cost scam campaigns.", 12)
        if host.count("-") >= 2 or len(host.split("." )[0]) > 28:
            _add(findings, "Unusual domain structure", "The domain has a pattern commonly seen in disposable phishing sites.", 9)
